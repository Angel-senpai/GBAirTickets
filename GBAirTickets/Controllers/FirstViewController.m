...
- (void)createContentDataArray {
    NSArray *titles = [NSArray arrayWithObjects:[@"About App" localize], [@"Flight tickets" localize], [@"Price map" localize], [@"Favorites" localize], nil];
    NSArray *contents = [NSArray arrayWithObjects:[@"Flight search application" localize], [@"Find the cheapest flights" localize], [@"View the price map" localize], [@"Save selected tickets to favorites" localize], nil];
    for (int i = 0; i < 4; ++i) {
  ...
  - (void)updateButtonWithIndex:(int)index {
      switch (index) {
          ...
          case 1:
          case 2:
              [_nextButton setTitle:@"CONTINUE" forState:UIControlStateNormal];
              _nextButton.tag = 0;
              break;
          case 3:;
