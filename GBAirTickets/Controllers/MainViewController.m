...
- (void)viewDidLoad {
    ...
    _placeContainerView.backgroundColor = [UIColor whiteColor];
        ...
        [_arrivalButton setTitle:[@"To" localize] forState: UIControlStateNormal];
        ...
        [_searchButton setTitle:[@"Search" localize] forState:UIControlStateNormal];
...
- (void)searchButtonDidTap:(UIButton *)sender {
    if (_searchRequest.origin && _searchRequest.destionation) {
            ...
            [[APIManager sharedInstance] ticketsWithRequest:self->_searchRequest withCompletion:^(NSArray *tickets) {
                [[ProgressView sharedInstance] dismiss:^{
                    if (tickets.count > 0) {
                        ...
                        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:[@"Oh!" localize] message:[@"Tickets not found" localize] preferredStyle: UIAlertControllerStyleAlert];
        ...
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:[@"Error" localize] message:[@"Not set place arrival or departure" localize] preferredStyle:UIAlertControllerStyleAlert];
        [alertController addAction:[UIAlertAction actionWithTitle:[@"Close" localize] style:(UIAlertActionStyleDefault) handler:nil]];
        [self presentViewController:alertController animated:YES completion:nil];
    }
}
